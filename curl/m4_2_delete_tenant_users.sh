...
grid_token=$(curl -s -k -X POST "https://192.168.0.80:443/api/v3/authorize" \
...
echo
curl -s -X GET "https://192.168.0.80:443/api/v3/grid/accounts" -H "accept: application/json"  -H "Authorization: Bearer $grid_token" -k  |jq -r '.data[] | "\(.name) \(.id)"'
echo ========

...
ADDR=192.168.0.80                  # Admin node / LB
TENANT_ID=$tenant_id           	    # tenant account ID
USER="root"                         # tenant admin/root user
read -rsp "Tenant user password: " PASS
#PASS="Netapp1!"		    # tenant password
...
id=$(curl -s -k -X GET "https://192.168.0.80/api/v4/org/users" -H "Authorization: Bearer $token" -H "Content-Type: application/json" | jq -r '.data[].id')

#create and get the user's access keys
curl -s -k -X POST   "https://192.168.0.80/api/v4/org/users/$id/s3-access-keys"   -H "Authorization: Bearer $token"   -H "Content-Type: application/json"   -d '{}'
...
#list the buckets
#aws s3 ls  --endpoint-url https://192.168.0.80:10443 --no-verify-ssl s3:// --profile griduser
